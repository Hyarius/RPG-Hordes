...
c_city_panel::c_city_panel(Vector2 panel_coord, Vector2 panel_size)
{
	_frame = c_frame(this);
	_frame.set_geometry(panel_coord, panel_size);
	_frame.box().set_back(Color(75, 60, 45));
	_frame.box().set_front(Color(100, 80, 60));
	_frame.activate();

	_city_name = c_text_label("City name", &_frame);
	...
	_city_defense_label = c_text_label("City defense :", &_frame);
	...
	_city_defense_value = c_value_label(-1.0f, &_frame);
	...
	_city_population_label = c_text_label("City population :", &_frame);
	...
	_city_population_value = c_value_label(-1.0f, &_frame);
